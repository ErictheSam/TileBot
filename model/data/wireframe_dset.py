...
class Line_Dataset(Dataset):
    ...
    def load_label(self, ann, do_aug):
        ...
        if can_load and self.cache_to_mem and label_cache_path  in self.cache_dict.keys():
            ...
            tp_mask = gen_TP_mask2(ann['norm_lines'], hsize, hsize,
                                   with_ext=self.cfg.datasets.with_centermap_extend)
            sol_mask, _ = gen_SOL_map(ann['norm_lines'], hsize, hsize,
                                      with_ext=False)

            # junction_map, line_map = gen_junction_and_line_mask(ann['norm_lines'],
            #                                                     hsize, hsize)

            label = np.zeros((2 * 7, hsize, hsize), dtype=np.float32)
            label[0:7, :, :] = sol_mask
            label[7:14, :, :] = tp_mask
...
#             img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
        ...
        if self.scale == True:
            hsize = hsize // 2
        ext_lines = get_ext_lines(ann['norm_lines'], hsize, hsize)
...
def LineDataset_collate_fn(batch):
    ...
    labels = np.zeros((batch_size, 14, h, w), dtype=np.float32)
    img_fns = []
    img_origin_list = []
    norm_lines_512_all = []
