...
from ._func import focal_neg_loss_with_logits, hausdorff_recall_loss#, weighted_bce_with_logits
...
def weighted_bce_with_logits(out, gt, pos_w=1.0, neg_w=30.0):
    ...
    # print('two_losses', loss_pos, loss_neg)
...
def len_and_angle_loss_func(pred_len, pred_angle, gt_len, gt_angle): # preserve
...
class LineSegmentLoss(nn.Module):
    ...
    def _m_match_loss_fn(self, p_lines, p_centers, p_scores, gt_lines, thresh):
        ...
        distance = distance1 + distance2
        near_inx = torch.argsort(distance, 1)[:, 0]  # most near one
    ...
    def matching_loss_func(self, pred_tp_mask, gt_line_512_tensor_list):
        ...
        for pred, gt_line_512 in zip(pred_tp_mask, gt_line_512_tensor_list):
            ...
            hsize = self.input_size
            if self.scale == True:
                hsize = hsize / 2
            pred_lines_128 = 128 * pred_lines / hsize
            pred_lines_128_swap = 128 * pred_lines_swap / hsize
            pred_center_ptss_128 = 128 * pred_center_ptss / hsize
    ...
    def tp_mask_loss(self, out, gt, gt_lines_tensor_512_list):
        ...
        if self.with_focal_loss:
            ...
            # print(center_loss, hausdorff_recall_loss(out_center, gt_center, alpha=4.0))
            # center_loss += hausdorff_recall_loss(out_center, gt_center, alpha=2.0)
    ...
    def sol_mask_loss(self, out, gt, sol_lines_512_all_tensor_list):
        out_center = out[:, 0, :, :]
        gt_center = gt[:, 0, :, :]
        # print(out_center.size(), gt_center.size())
