...
def get_args():
    ...
    parser.add_argument("--config",
                        default= os.path.dirname(__file__)+ '/configs/enet.yaml',
                        type=str,
                        help="")
...
def train(cfg, dev):
    ...
    model = build_model(cfg, dev).cuda(dev)
    ...
    if os.path.exists(cfg.train.enet_load_from):
        model.enet_backbone.load_state_dict(torch.load(cfg.train.enet_load_from,map_location=device),strict=False)
...
def test(cfg, dev):
    ...
    if os.path.exists(cfg.train.enet_load_from):
        model.enet_backbone.load_state_dict(torch.load(cfg.train.enet_load_from,map_location=device),strict=False)
    logger = TxtLogger(cfg.train.save_dir + "/test_logger.txt")
