...
class Simple_MLSD_Learner():
    ...
    def step(self, step_n,  batch_data : dict):
        imgs  = batch_data["xs"].cuda(device)
        label = batch_data["ys"].cuda(device)
    ...
    def val(self, model, thresh, val_dataloader : DataLoader):
...
#             batch_outputs[:, 1:5, :,: ] = label[:, 1:5, :,: ]
            ...
            for outputs, gt_lines_512 in zip(batch_outputs, batch_data["gt_lines_512"]):
                ...
                center_ptss, pred_lines, _, scores = \
                    deccode_lines_TP(outputs, thresh - 0.01, min_len, topk+100, 5)
                ...
                pred_lines_128 = 128 * pred_lines / self.input_size
    ...
    def train(self, train_dataloader : DataLoader,
        ...
        for self.epo in range(epoches):
            ...
            if self.epo == self.cfg.train.defroze_step:
                self.model.defroze()
                mid_model_path = os.path.join(self.save_dir, 'middle.pth')
                torch.save(self.model.state_dict(), mid_model_path)
            ...
            for batch in data_iter:
                ...
                status = '[{0}] lr= {1:.6f} loss= {2:.3f} avg = {3:.3f},c: {4:.3f}, d: {5:.3f}, ' \
                         'm:{6:.3f},m_r:{7:.2f} '.format(
                    self.epo + 1,
            ...
            if self.epo > self.cfg.val.val_after_epoch:
                ## val
                m = self.val(self.model, self.cfg.decode.score_thresh, val_dataloader)
                fscore = m['sAP10']
    ...
    def test(self,
        ...
        #                                             loss_scale=1.0
        #                                             )
        thresh_list = [0.01, 0.02, 0.05, 0.10, 0.20, 0.30]
        dct_list = []
        for thr in thresh_list:
            dct_list.append(self.val(self.model, thr, val_dataloader))
        print(dct_list)